...
object ClientMetrics extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      System.out.println("[APM] - Opts: " + args.toList.asJava)
      processCommand(opts)
    } catch {
  ...
  class ConfigCommandParser(args: Array[String]) extends CommandDefaultOptions(args) {
    ...
    def checkArgs(): Unit = {
      ...
      val actions = Seq(addOp, deleteOp, listOp).count(options.has)
      if (actions != 1)
        CommandLineUtils.printUsageAndExit(parser, "Command must include exactly one action: --add, --delete, --list")

      // check add args
//       CommandLineUtils.checkInvalidArgs(parser, options, addOp, Set(deleteOp, listOp).asJava)

      // check delete args
//       CommandLineUtils.checkInvalidArgs(parser, options, deleteOp, Set(addOp, listOp, matchSelector, metric, interval).asJava)

      // check list args
//       CommandLineUtils.checkInvalidArgs(parser, options, listOp, Set(addOp, deleteOp, matchSelector, metric, interval, block).asJava)
  ...
  object ClientMetricService {
    ...
    case class ClientMetricService private (adminClient: Admin) extends AutoCloseable {
      ...
      def addMetrics(addMetricsOpts: AddMetricsOptions): Unit = {
        ...
        System.out.println("[APM] - configs to be added: {}", configsToBeAdded)
