 ...
 * are known. Care should be taken to limit the scope of these internal implementation details
 * to keep the code more flexible.
 *
 * <p/>
 *
...
public interface Emitter extends Closeable {
     ...
     * manager should respect this and not just call the {@link #emitMetric(SinglePointMetric)} directly.
     *
     ...
     *
     * @param metric {@link SinglePointMetric}
     ...
     * method should be invoked by the telemetry reporter before calls to {@link #emitMetric(SinglePointMetric)}
     * are made. The telemetry reporter should not invoke this method more than once.
     */
    default void init() {
        // Do nothing...
    }
     ...
     * resources. This method should ideally be invoked only once by the telemetry reporter, but
     * under some edge cases it might end up being called more than once.
     *
     * <p/>
     *
     * Calls to {@link #emitMetric(SinglePointMetric)} once this method has been invoked should be expected to
