...
import org.apache.kafka.clients.consumer.internals.PrototypeAsyncConsumerContext;
...
public class ApplicationEventProcessor<K, V> {
    ...
    public ApplicationEventProcessor(final PrototypeAsyncConsumerContext consumerContext,
                                     final RequestManagers<K, V> requestManagers) {
        this.log = consumerContext.logContext.logger(ApplicationEventProcessor.class);
    ...
    private boolean process(final NoopApplicationEvent event) {
        return consumerContext.backgroundEventQueue.add(new NoopBackgroundEvent(event.message));
    }
