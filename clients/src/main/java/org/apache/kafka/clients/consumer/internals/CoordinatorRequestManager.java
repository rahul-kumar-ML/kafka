...
public class CoordinatorRequestManager implements RequestManager {
    ...
    public CoordinatorRequestManager(final PrototypeAsyncConsumerContext consumerContext,
                                     final long retryBackoffMs,
                                     final String groupId) {
        Objects.requireNonNull(groupId);
        this.log = consumerContext.logContext.logger(this.getClass());
    ...
    private NetworkClientDelegate.UnsentRequest makeFindCoordinatorRequest(final long currentTimeMs) {
        ...
        unsentRequest.future().whenComplete((clientResponse, throwable) -> {
            long responseTimeMs = consumerContext.time.milliseconds();
    ...
    private void onFailedResponse(final long currentTimeMs, final Throwable exception) {
        ...
        if (exception == Errors.GROUP_AUTHORIZATION_FAILED.exception()) {
            ...
            consumerContext.backgroundEventQueue.add(new ErrorBackgroundEvent(GroupAuthorizationException.forGroupId(this.groupId)));
            return;
        }

        log.warn("FindCoordinator request failed due to fatal exception", exception);
        consumerContext.backgroundEventQueue.add(new ErrorBackgroundEvent(exception));
    }
