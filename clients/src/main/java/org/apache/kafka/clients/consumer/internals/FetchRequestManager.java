...
public class FetchRequestManager<K, V> extends AbstractFetch<K, V> implements RequestManager {
    ...
    FetchRequestManager(final PrototypeAsyncConsumerContext consumerContext,
                        final ConsumerMetadata metadata,
                        final SubscriptionState subscriptions,
                        final FetchConfig<K, V> fetchConfig,
                        final FetchMetricsManager metricsManager,
                        final long retryBackoffMs) {
        super(consumerContext.logContext,
                metadata,
                subscriptions,
                fetchConfig,
                metricsManager,
                createFetchClientChecker(),
        ...
        for (Map.Entry<Node, FetchSessionHandler.FetchRequestData> entry : fetchRequestMap.entrySet()) {
            ...
            unsentRequest.future().whenComplete((clientResponse, t) -> {
                if (t != null) {
                    ...
                    consumerContext.backgroundEventQueue.add(new ErrorBackgroundEvent(t));
                } else {
    ...
    private static FetchClientChecker createFetchClientChecker() {
        return new FetchClientChecker() {
            @Override
